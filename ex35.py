from sys import exit

def gold_rm():
    print('This room is full of gold.  How much do you take?')

    choice = input('> ')
    if '0' in choice or '1' in choice:
        qty = int(choice)
    else: dead('Man, learn to type a number.')

    if qty > 50:
        print('Nice, you\'re not greedy, you win!')
        exit(0)
    else: dead('You greedy bastard!')


def bear_rm():
    print('There is a bear here.')
    print('The bear has a bunch of honey.')
    print('The fat bear is in front of another door.')
    print('How are you going to move the bear?')
    bear_mvd = False

    while True:
        choice = input('> ')

        if choice == 'take honey':
            dead('The bear looks at you then slaps your face off.')
        elif choice == 'taunt bear' and not bear_mvd:
            print('The bear has moved from the door.')
            print('You can go thru it now.')
            bear_mvd = True
        elif choice == 'taunt bear' and bear_mvd:
            dead('The bear gets pissed off and chews your leg off.')
        elif choice == 'open door' and bear_mvd:
            gold_rm()
        else: print('I got no idea what that means.')


def cthulhu_rm():
    print('Here you see the great evil Cthulhu.')
    print('He, it, whatever stares at you and you go insane.')
    print('Do you flee for your life or eat your head?')

    choice = input('> ')

    if 'flee' in choice: start()
    elif 'head' in choice: dead('Well that was tasty')
    else: cthulhu_rm()


def dead(why):
    print(f'{why} Good job!')
    exit(0)


def start():
    print('You are in a dark room.')
    print('There is a door to your left and right.')
    print('Which one do you take?')

    choice = input('> ')

    if choice == 'left': bear_rm()
    elif choice == 'right': cthulhu_rm()
    else: dead('You stumble around the room until you starve.')


start()
